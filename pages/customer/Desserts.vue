<template>
    <v-app>
    <v-container>
        <v-row><v-col cols="12"><v-card ><v-card-title class="text-center">Desserts</v-card-title ></v-card></v-col></v-row>
        
            <v-row>
        <v-col cols="6" v-for="menuItem in dessertMenuItems" :key="menuItem.id">
            <v-card >
...
export default {
    ...
    computed: {
        dessertMenuItems() { return this.tableData.filter( (menuItem) => menuItem.menu_cat === "dessert" ); },
    },
};
</script>